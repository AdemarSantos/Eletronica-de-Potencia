...
%         h      (0.0001)
%         w   (376.9911184307752) // 2*pi*60
%         iw   (0.002652582384865) // 1/w 

%         arg = w_ref*h;
%         F1 = cos(arg);
%         F2 = sin(arg)*1/w_ref;
%         F3 = 2.0*kii*sin(arg)*1/w_ref;
%         F4= -w_ref*sin(arg);
%         F5= cos(arg);
...
% 
%         if(xa1 >  max_vg) xa1 =  max_vg;
%         if(xa1 < -max_vg) xa1 = -max_vg;
% 
%         vg1_ref = xa1 + 2.*kpi*ei1;
% 
%         if(vg1_ref >  max_vg) vg1_ref  =  max_vg;
%         if(vg1_ref < -max_vg) vg1_ref  = -max_vg;
...
figure(10)
title('Tensão do Barramento [V]')
plot(Ts,vcs,Ts,vc_refs,'r--')
