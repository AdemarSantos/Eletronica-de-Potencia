...
Vdc3L_ref = 100;         % Tensão de referência do barramento CC do conversor 3L
Vdc2L_ref = 100;         % Tensão de referência do barramento CC do conversor 2L

Vdc3L = 100;             % Tensão atual do barramento CC do conversor 3L
Vdc2L = 100;             % Tensão atual do barramento CC do conversor 2L
...
while t<tf
    ...
    if t >= ttriangle
        ...
        vxmaxgba =  (Vdc2L+Vdc3L/2) - vg_ref;
        vxmingba = -(Vdc2L+Vdc3L/2) - vg_ref;
        vxmaxl0  =  Vdc3L/2 - max([vl_ref,0]);
        vxminl0  = -Vdc3L/2 - min([vl_ref,0]);
        ...
        vymaxga0 =  Vdc3L/2 - vgba_ref/2;
        vyminga0 = -Vdc3L/2 - vgba_ref/2;
        vymaxgb  =  Vdc2L + vgba_ref/2;
        vymingb  = -Vdc2L + vgba_ref/2;
    ...
    % 2L
    vgb10b = (2*qgb1 - 1)*(Vdc2L/2); 
    vgb20b = (2*qgb2 - 1)*(Vdc2L/2);
    
    % 3L
    vha0a = (2*qha - 1)*(Vdc3L/2);
    vga0a = (2*qga - 1)*(Vdc3L/2);
    vla0a = (2*qla - 1)*(Vdc3L/2);
...
figure(4)
plot(Ts,vg_refs,Ts,-vgb_refs + (vga0a_refs-vha0a_refs),'r--')
...
figure(5)
plot(Ts,vl_refs,Ts,vla0a_refs-vha0a_refs,'r--')
...
figure(6)
plot(Ts,vga0a_refs,Ts,vha0a_refs,Ts,vla0a_refs,'b--')
title('v*l vs Modelo','FontSize',18)
legend('vga0','vha0','val0','FontSize',16)
xlabel("Tempo (s)")
ylabel("Tensão (V)")

figure(6)
plot(Ts,vgb10b_refs,Ts,vgb20b_refs,'b--')
title('v*l vs Modelo','FontSize',18)
