...
% Rl = 15.5;               % Resistência da carga
% Ll = 30.8E-3;            % Indutância da carga
%% ---------------- Condições do Sistema ----------------------------
Vgm = 220;       % Tensão RMS da rede (eg)
Vl_ref = 220;    % Tensão RMS de referência braço L

alpha = 45*(pi/180);     % Fase da tensão de referência braço L
...
%% Cálculo do valor eficaz da corrente drenada (ig e vg em fase)
Ig_e = min(roots([Rg -Vgm Pl]));
%Ig_e = 9.118;
Xg = Lg*w_ref;
vg_e = (Vgm - Ig_e*Rg) - 1j*(Xg*Ig_e);  

%fase_e = atan((-Ig_e*Xg)/(Vgm - Ig_e*Rg));
%Vg_e = -Ig_e*Xg/sin(fase_e);
...
tsave0 = 0/60.;         % Tempo inicial de gravação
...
control = 0;             % Estado da banda de histerese
...
atrasoPWM = 0.5*2.*pi*f_ref/ftriangle;
%% ---------------- Início do Looping ----------------

while t<tf
    t = t + h;

    eg = Vgm*sqrt(2)*cos(w_ref*t);                     % Tensão da rede
...
%         Vc3L_error = (Vc3L_ref+Vc2L_ref)/2 - (Vc3L+Vc2L)/2;
        ...
        vg_ref_e = Vg_e*sqrt(2)*cos(w_ref*t + fase_e + atrasoPWM);
%         vg_ref = vg_ref_e;
        vg_ref = eg - ig*Rg - (Lg/htriangle)*ig_error;
        vl_ref = Vl_ref*sqrt(2)*cos(w_ref*t + alpha);        
...
%         control =  1; % Carregar Sempre
%         control = -1; % Descarregar Sempre
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % Corrente média nos barramentos
        % 2L
    ...
    if tsave <= t
        ...
        %%%
        vg_ref_es(n) = vg_ref_e;
        
        ic3Ls(n) = ic3L;
        Igs(n) = Ig;
        
        ic2L_meds(n)=ic2L_med;
        ic3L_meds(n)=ic3L_med;
        
    end
end
%% ---------------- Fim do Looping ----------------
%% Simulação com parâmetro variado
