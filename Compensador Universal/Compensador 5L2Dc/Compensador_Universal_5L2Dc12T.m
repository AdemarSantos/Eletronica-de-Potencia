...
Igr2 = min(roots([Rg -0.9*2*Ea/2 Pl]));
vgr2 = (Vgm-Igr*Rg) - 1j*(Igr*Xg);
thetagr2 = angle(vgr);
Vgr2 = real(vgr);
...
mi = 1.05;               % Inclinação da reta de carregamento do conv. 3L
...
while t < tf
    ...
    if t >= ttriangle
        ...
        vgr2_ref = Vgr2*cos(w_ref*t + thetagr2);
        vgc_ref = eg - ig*Rg - (Lg/htriangle)*ig_error;
        vgL_ref = Vg_ref*cos(w_ref*t);
        vl_ref = Vl_ref*cos(w_ref*t + alpha); 
        
        vg_ref = vgc_ref;
        
        if (Vgm > Ea && Vc3L < 0.95*2*Ea && z == 1) 
            vg_ref = vgL_ref;
        elseif Vc3L > 0.95*2*Ea && Vgm > Ea
            z = -1;
        elseif (Vgm < Ea && Vc3L > 1.05*Ea && z == 1) 
            %vg_ref = vgL_ref;
            z = 0;
        elseif Vc3L < 1.05*Ea && Vgm < Ea
            z = 0;
        end
