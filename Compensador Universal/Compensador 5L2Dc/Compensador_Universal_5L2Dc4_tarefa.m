...
% Igr = min(roots([Rg -Vgm/2 Pl]))-3.169;
...
plot(Ts,egs,'r-','LineWidth',2),zoom
title('Tensão da rede','FontSize',18)
legend('eg','FontSize',16)
xlabel("Tempo (s)")
ylabel("Tensão (V)")
% axis([0 tf -1.1 1.1])


figure('name','Tensão de referência do braço G')
plot(Ts,vg_refs,Ts,vgr_refs,'r--','LineWidth',2),zoom
title('Tensão de referência controlada e calculada','FontSize',18)
legend('Vg*','Vgr*','FontSize',16)
xlabel("Tempo (s)")
ylabel("Tensão (V)")
% axis([0 tf -1.1 1.1])
%% Tensões Vg e Vl
