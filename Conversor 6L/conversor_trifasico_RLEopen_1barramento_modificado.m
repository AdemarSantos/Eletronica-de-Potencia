...
while t<=tmax,
    ...
    if t>=tpwm
            ...
            vbaref_max =  Eab - max(vgref);
            vbaref_min = -Eab - min(vgref);
            vbaref_med = (vbaref_max+vbaref_min)/2;

            %geração das tensões de pólo de referencia 
            vg10ref=vg1ref+vbaref_med;
            vg20ref=vg2ref+vbaref_med;
            vg30ref=vg3ref+vbaref_med;
...
figure(7),plot(tempo,tensaog3ref,tempo,tensaog3_m,tempo,tensao3),grid,zoom
