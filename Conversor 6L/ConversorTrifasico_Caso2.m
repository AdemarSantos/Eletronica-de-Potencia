...
Ea = 2*E/3; %%% Tensão do barramento do conversor A              
Eb = E/3;   %%% Tensão do barramento do conversor B
...
vtri = Eab/3;              % Tensão máxima da onda triangular
dtri = 2*vtri/htri;        % Derivada da onda triangular (dV/dT)
...
while t <= tf
    ...
    elseif vg10_ref <= Eab/3 && vg10_ref >= -Eab/3
        if vg10_ref >= vtri2
            q1a = 1;
            q1b = 1;            
        else
    ...
    elseif vg20_ref <= Eab/3 && vg20_ref >= -Eab/3
        if vg20_ref >= vtri2
            q2a = 1;
            q2b = 1;            
        else
    ...
    elseif vg30_ref <= Eab/3 && vg30_ref >= -Eab/3
        if vg30_ref >= vtri2
            q3a = 1;
            q3b = 1;            
        else
...
plot(Ts,triangle1,Ts,triangle2,Ts,triangle3,'LineWidth',1.5),zoom
title('Tensões de saída do conversor','FontSize',18)
legend('Triangular','Triangular Positiva','Triangular Negativa')
xlabel("Tempo (s)")
ylabel("Tensão (V)")
